...
public final class ModelUtils {
    ...
    public static function adaptVoicesToStavesNumberIn(part:ProjectData):void {
        ...
        // are we hard-coding the limitation of a staff accepting at most `two` voices; instead, this limit is
    ...
    */
    ...
    /**
     * Compares score objects by their `relevance` fields, numeric, ascending.
     *
     * @param    scoreA
     ...
     *            An Array of strings.
     *
     * @return    An Array of strings with all the occurrences of the `value not set`
     *            substring removed.
     */
    ...
    public static function sortStavesByPartOrder(staves:Array):void {
        if (_unifiedPartsList != null) {
            ...
            var mustSupplementOffset:Boolean = false;
            for (var j:int = 0; j < staves.length; j++) {
                ...
                if (mustSupplementOffset) {
                    mustSupplementOffset = false;
    ...
    /**
     * Note: possible duplicate of: `compareVoiceNodes()`. To investigate.
     */
    ...
    private static function _exportUnifiedPartsList():Array {
        ...
        for (var sectionName:String in _partsPerSection) {
            ...
            for (var partName:String in partsInCurrSect) {
                if (!(partName in unifiedDict)) {
                    unifiedDict[partName] = [];
                }
                ...
                for (var partInstanceIdx:int = 0; partInstanceIdx < partInstancesInSection.length; partInstanceIdx++) {
                    ...
                    var unifiedListOfCurrentPart:Array = (unifiedDict[partName] as Array);
                    // However, if two violins play in the first section, and three
                    ...
                    var havePartInstanceAtCurrentIndex:Boolean = (unifiedListOfCurrentPart[partInstanceIdx] !== undefined);
                    if (!havePartInstanceAtCurrentIndex) {
                        unifiedListOfCurrentPart[partInstanceIdx] = partInstanceData;
                    } else {
                        // We also must provide a meaningful view of the number of staves
                        // the parts use. If we have a harp using only one staff in the
                        // first section, but two in the second, we'll setup a two-staves
                        // harp score.
                        var currentNumStaves:int = (unifiedListOfCurrentPart[partInstanceIdx][DataFields.PART_NUM_STAVES] as int);
                        var newNumStaves:int = (partInstanceData[DataFields.PART_NUM_STAVES] as int);
                        if (newNumStaves > currentNumStaves) {
                            unifiedListOfCurrentPart[partInstanceIdx][DataFields.PART_NUM_STAVES] = newNumStaves;
                        }
                    }
                }
            }
        }
        var unifiedPartsList:Array = [];
        for (var key:String in unifiedDict) {
            var value:Array = (unifiedDict[key] as Array);
            ...
            unifiedPartsList = unifiedPartsList.concat(value);
        }
        unifiedPartsList.sort(_byChildIndex);
        return unifiedPartsList;
    }

    /**
     * Sorting function, used by Array.sort() inside `_exportUnifiedPartsList()` to ensure parts are exported in a
     * consistent order. Sorts parts ascending by the child index they respectively have in the data model..
     */
