...
public final class MusicUtils {
     ...
     * `CHROMATIC_SCALE`. For instance, it will convert a `C double flat` to an `A sharp`.
     *
     * @param    note
     *            The note to convert.
     *
    ...
    public static function canonicalizeNote(note:ProjectData, forceToFirstOctave:Boolean = false):ProjectData {
        if (note != null) {
            ...
            // The octave index is somewhat dispensable, as we are primarily looking to get rid of
     ...
     *
     * @return    An array with 3 values, representing respectively:
     *            [0] Pitch name, e.g. "A";
    ...
    /**
     * Converts a ProjectData of type "note" to its equivalent MIDI pitch.
     *
     * @param    note
     *            The note to be converted into its MIDI number.
     *
     * @param        forceDiatonic
     *                If true, the returned MIDI note will reflect the "base pitch", ignoring the
     *                current note alteration, so that, e.g., a C# note will produce MIDI `60`, NOT `61`
     *                (as if that note was a `C` instead).
     *
     ...
     *            If true, duplicates existing Note nodes in each Clusters and transposes the dupes,
     *            effectively creating parallel intervals. Existing pitches are not supplicated in
     *            the process.
     */
    public static function transposeBy(clusters:Array, transpositionInterval:int, maintainExistingNotes:Boolean = false):void {
        if (clusters && clusters.length && transpositionInterval) {
            for (var i:int = 0; i < clusters.length; i++) {
                ...
                // Eventually, we add all cloned notes that were set aside for the current Cluster.
    ...
    public static function consolidatePrimeIntervals(sourceMaterial:ProjectData,
        ...
        // Group together eligible Clusters in batches, and attempt to resolve their gross duration to one
        ...
        var processCurrentBatch:Function = function ():void {
            ...
            var lastClusterInBatch:ProjectData;
        ...
        while (rawClusters.length > 0) {
            ...
            if (referenceCluster) {
                ...
                // Encountering a rest after another rest represents an eligible situation.
                ...
                if ((cluster.numDataChildren == 0) && (referenceCluster.numDataChildren == 0)) {
                    if (!_isFirstInVoice(cluster) && _areInSameTuplets(referenceCluster, cluster)) {
                        ...
                        currentClustersBatch.push(cluster);
                            ...
                            // than the reference Cluster are not eligible. Encountering such a Cluster seals
                ...
                // state and forces it to be processed immediately.
                    ...
                    continue;
                        ...
                        // to a number of restrictions, see below) makes for an eligible situation. By best
                // effort, they will be consolidated.
                    ...
                    // the current batch in its current state and forces it to be processed immediately.
                        ...
                        // are not eligible. Encountering such a Cluster seals the current batch in its
            ...
            // "processCurrentBatch()" function can "expel" Clusters from the current batch if they fail to
        ...
        // Explicitly process the last batch of Clusters, if any. If we are at the end of the "sourceMaterial",
    ...
    private static function _areInSameTuplets(clusterA:ProjectData, clusterB:ProjectData):Boolean {
        ...
        var aStartsTuplet:Boolean = clusterA.getContent(DataFields.STARTS_TUPLET);
        var bStartsTuplet:Boolean = clusterB.getContent(DataFields.STARTS_TUPLET);
    ...
    private static function _hasTie(cluster:ProjectData):Boolean {
        for (var i:int = 0; i < cluster.numDataChildren; i++) {
            var note:ProjectData = ProjectData(cluster.getDataChildAt(i));
            if (note.getContent(DataFields.TIES_TO_NEXT_NOTE)) {
                return true;
            }
        }
        return false;
    ...
    private static function _buildChromaticScale():Array {
        var scale:Array = [];
        var template:Array = [[PitchNames.C, PitchAlterationTypes.NATURAL], [PitchNames.C, PitchAlterationTypes.SHARP], [PitchNames.D, PitchAlterationTypes.NATURAL], [PitchNames.D, PitchAlterationTypes.SHARP], [PitchNames.E, PitchAlterationTypes.NATURAL], [PitchNames.F, PitchAlterationTypes.NATURAL], [PitchNames.F, PitchAlterationTypes.SHARP], [PitchNames.G, PitchAlterationTypes.NATURAL],
     ...
     * pitch name and pitch alteration. This allows to gracefully equate
